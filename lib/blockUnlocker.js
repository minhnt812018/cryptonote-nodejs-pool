...
function runInterval(){
    async.waterfall([
		...
		redisClient.zrevrange(config.coin + ':blocks:matured', 0, 100, function(error, members){
			var rewardheight = 0;
			var avgreward = 0;
			for (var i = 0; i < members.length; i++){
			      var data = members[i].split(':');
			      rewardheight += parseFloat(data[5])/config.coinUnits;
		...
		var mScores =  Object.keys( block.workerScores).reduce( function( sum, key ){
                	return sum + parseFloat( block.workerScores[key] );
              	 }, 0 );
		    var oScores = parseFloat(block.score);
		    //var totalScore = mScores > oScores?mScores :oScores;
                    var totalScore = mScores;
		    log('info', logSystem, 'total: %d - %d - %d', [mScores ,oScores , totalScore ]);
