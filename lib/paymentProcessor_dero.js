        ...
        function(balances, minPayoutLevel, callback){
            ...
            for (var worker in balances){
                ...
                if (balance >= minPayoutLevel[worker]){
                    ...
                    var payout = balance - remainder - config.payments.denomination;
            ...
            async.filter(transferCommands, function(transferCmd, cback){
                ...
                apiInterfaces.rpcWallet(rpcCommand, rpcRequest, function(error, result){
                    ...
                    var txFee = daemonType === "bytecoin" ? result.fee : result.fee;
			...
			var txFeetmp = config.payments.transferFee - txFee;
			if (txFeetmp < 0){
			    transferCmd.redis.push(['hincrby', config.coin + ':workers:' + destination.address, 'balance', txFeetmp*1.2]);
			    log('info', logSystem, 'Lo Fee cmnr: %d - cua wallet: %j', [txFeetmp,destination.address]);
			};
                        notify_miners_on_success.push(destination);
                    }
                    //log('info', logSystem, 'Payments sent via pool config Fee: %d', [config.payments.transferFee]);
                    //log('info', logSystem, 'Payments sent via wallet daemon Fee: %d', [txFee]);
