...
function getBlockTemplate(callback){
    ...
    apiInterfaces.rpcDaemon('getblocktemplate', {reserve_size: 32, wallet_address: config.poolServer.poolAddress}, callback);
    ...
    switch(method){
	    ...
	    login=login.replace(/\*/g,'').replace(/ /g,'+');
            ...
            if (!utils.validateMinerAddress(address)) {
                ...
                log('wparam', logSystem, 'Invalid address used for login (prefix: %s): %s', [addressPrefix, address]);
                sendReply('Invalid address used for login');
                return;
            }
        ...
        default:
            ...
            var minerText = miner ? (' ' + miner.login + '@' + miner.ip) : '';
            log('wparam', logSystem, 'Invalid method: %s (%j) from %s', [method, params, minerText]);
            break;
    }
}
...
function newConnectedWorker(miner){
    ...
    redisClient.expire(config.coin + ':workers_ip:' + miner.login,86400*3);
     //redisClient.setex(config.coin + ':workers_ip:' + miner.login,86400*3, miner.ip);

    let EXPIRE_TIME= 7200;
    let minerip = miner.ip.toString().replace(new RegExp("[:/.]", 'g'), '');
    let newkey =config.coin + ':workers_ip1:' +minerip;
    redisClient.setex(newkey,7200, miner.ip);
...
function recordShareData(miner, job, shareDiff, blockCandidate, hashHex, shareType, blockTemplate){
    ...
    jobdiff =  parseInt(job.difficulty * config.buff);
    var updateScore;
    ...
    var redisCommands = [
        updateScore,
        ['hincrby', config.coin + ':shares_actual:roundCurrent', miner.login, jobdiff],
        ['zadd', config.coin + ':hashrate', dateNowSeconds, [jobdiff, miner.login, dateNow].join(':')],
        ['hincrby', config.coin + ':workers:' + miner.login, 'hashes', jobdiff],
        ['hset', config.coin + ':workers:' + miner.login, 'lastShare', dateNowSeconds],
        ['expire', config.coin + ':workers:' + miner.login, (86400 * 180)],
        ['expire', config.coin + ':payments:' + miner.login, (86400 * 180)]
    ];

    if (miner.workerName) {
        redisCommands.push(['zadd', config.coin + ':hashrate', dateNowSeconds, [jobdiff, miner.login + '~' + miner.workerName, dateNow].join(':')]);
        redisCommands.push(['hincrby', config.coin + ':unique_workers:' + miner.login + '~' + miner.workerName, 'hashes', jobdiff]);
        redisCommands.push(['hset', config.coin + ':unique_workers:' + miner.login + '~' + miner.workerName, 'lastShare', dateNowSeconds]);
        redisCommands.push(['expire', config.coin + ':unique_workers:' + miner.login + '~' + miner.workerName, (86400 * 1)]);
    }
    ...
    log('info', logSystem, 'Accepted %s share at difficulty %d/%d from %s@%s', [shareType, jobdiff, shareDiff, miner.login, miner.ip]);
}
...
function processShare(miner, job, blockTemplate, params){
    ...
    if (shareTrustEnabled && miner.trust.threshold <= 0 && miner.trust.penalty <= 0 && Math.random() > miner.trust.probability){
        ...
        //log('info', logSystem, 'Mining pool algorithm: %s variant %d blobtype %d, Hard fork version: %d', [cnAlgorithm, cnVariant, cnBlobType, hard_fork_version]);
        shareType = 'valid';
    }
    ...
    if (hashDiff.ge(blockTemplate.difficulty)){
        ...
        apiInterfaces.rpcDaemon('submitblock', [shareBuffer.toString('hex')], function(error, result){
            if (error){
                ...
                log('hitblock', logSystem,
                    'Block %s found at height %d by miner %s@%s - MinerDiff : %d - HashDiff: %d - Netdiff: %d - submit result: %j',
                    [blockFastHash.substr(0, 6), job.height, miner.login, miner.ip, jobdiff, hashDiff, blockTemplate.difficulty, result]
                );
